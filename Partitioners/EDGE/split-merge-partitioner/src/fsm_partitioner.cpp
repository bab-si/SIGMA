...
void FsmPartitioner::split()
{
    ...
    {
        total_time = split_partitioner->total_time;
    ...
    const eid_t max_occupied = *std::max_element(occupied.begin(), occupied.end());
    ...
    metrics_file << "  \"node_balance\": " << (double)max_nodes / ((double)total_mirrors / num_partitions) << ",\n";
    metrics_file << "  \"edge_balance\": " << (double)max_occupied / ((double)num_edges / num_partitions) << ",\n";
    metrics_file << "  \"replication_factor\": " << (double)total_mirrors / num_vertices << ",\n";
