...
#include "rand_partitioner.hpp"
#include "graph.hpp"
#include "conversions.hpp"

RandPartitioner::RandPartitioner(std::string basefilename, bool need_k_split)
...
void RandPartitioner::split()
{
    ...
    while (num_remaining_edges > 0) { // edges to be read
        ...
        for (eid_t i = 0; i < chunk_size; ++i, ++num_edges_read) {
            ...
            bucket = num_edges_read % num_partitions;
    ...
    calculate_stats();
    write_edge_metrics_json(
        basefilename, "rand", num_partitions, num_vertices, num_edges, occupied,
        is_boundarys, partition_time.get_time(), total_time.get_time());
}
