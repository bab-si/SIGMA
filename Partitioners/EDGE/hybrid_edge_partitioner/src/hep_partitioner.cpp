...
void HepPartitioner::split()
{
    ...
    std::vector<size_t> node_counts(p, 0);
    size_t total_mirrors = count_mirrors();
    for (int i = 0; i < p; ++i) {
        node_counts[i] = is_boundarys[i].popcount();
    }
    ...
    metrics_file << "{\n";
    metrics_file << "  \"algorithm\": \"hybrid_edge_partitioner\",\n";
    metrics_file << "  \"dataset\": \"" << basefilename << "\",\n";
    metrics_file << "  \"k\": " << p << ",\n";
    metrics_file << "  \"partitioning_type\": \"edge\",\n";
    metrics_file << "  \"num_parts\": " << p << ",\n";
    metrics_file << "  \"num_nodes\": " << num_vertices << ",\n";
    metrics_file << "  \"num_edges\": " << num_edges << ",\n";
    metrics_file << "  \"node_balance\": " << (double)max_nodes / ((double)total_mirrors / p) << ",\n";
    metrics_file << "  \"edge_balance\": " << (double)max_occupied / ((double)num_edges / p) << ",\n";
