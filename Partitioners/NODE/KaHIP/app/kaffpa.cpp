...
void write_node_metrics_json(const std::string& partition_output,
                             ...
                             double edge_balance,
                             double core_time) {
        std::string artifact_dir = partition_output + ".artifacts";
        ensure_dir_exists(artifact_dir);

        std::ofstream metrics(artifact_dir + "/metrics.json");
        metrics << "{\n";
        metrics << "  \"algorithm\": \"kaffpa\",\n";
...
int main(int argn, char **argv) {
        ...
        double partitioning_time = t.elapsed();
        std::cout <<  "time spent for partitioning " << partitioning_time  << std::endl;
        ...
        double edge_cut_ratio = G.number_of_edges() == 0 ? 0.0 : static_cast<double>(edge_cut) / G.number_of_edges();
        write_node_metrics_json(filename.str(),
                                basename_of(graph_filename),
                                partition_config.k,
                                G.number_of_nodes(),
                                G.number_of_edges(),
                                edge_cut,
