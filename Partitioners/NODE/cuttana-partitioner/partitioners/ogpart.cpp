...
public:
    ...
    void verify()
    {
        ...
        result.node_balance = imbalance_vertex;
        result.edge_balance = imbalance_edge;
        result.core_time = result.program_timer.get_total_time() / 1e9;
...
{
    Timer e2e_timer("End-to-end timer");
    e2e_timer.tick();
    program_timer.tick();
#ifdef CV
    cout << "CV MODE!" << endl;
    ...
    ogpart.verify();
    e2e_timer.untick();
    result.e2e_time = e2e_timer.get_total_time() / 1e9;
    result.core_time = result.e2e_time;

    {
        string metrics_file =
            "partitioned_files/metrics_" + result.dataset + "_" + to_string(result.part_count) + "_" + to_string(result.sub_part_count) + ".json";
        ofstream out(metrics_file);
        out << "{\n";
        out << "  \"algorithm\": \"cuttana\",\n";
        out << "  \"dataset\": \"" << result.dataset << "\",\n";
        out << "  \"k\": " << result.part_count << ",\n";
        out << "  \"partitioning_type\": \"node\",\n";
        out << "  \"num_parts\": " << result.part_count << ",\n";
        out << "  \"num_nodes\": " << result.vertex_count << ",\n";
        out << "  \"num_edges\": " << result.edge_count << ",\n";
        out << "  \"edge_cut\": " << result.phase2_edge_cut << ",\n";
        out << "  \"edge_cut_ratio\": "
            << (result.edge_count ? static_cast<double>(result.phase2_edge_cut) / result.edge_count : 0.0) << ",\n";
        out << "  \"node_balance\": " << result.node_balance << ",\n";
        out << "  \"edge_balance\": " << result.edge_balance << ",\n";
        out << "  \"core_time\": " << result.core_time << "\n";
        out << "}\n";
    }
