...
//       }
template <typename T>
T read_int(char *&file_ptr, char *end)
{
    while (file_ptr < end && !isdigit(static_cast<unsigned char>(*file_ptr)))
        file_ptr++;

    if (file_ptr >= end)
        throw std::runtime_error("Unexpected EOF while skipping non-digits");

    if (file_ptr >= end)
    {
        throw std::runtime_error("EOF in read_int");
    }

    T num = 0;
    while (file_ptr < end && isdigit(*file_ptr))
    {
        num = num * 10 + (*file_ptr - '0');
        file_ptr++;
    }
...
inline double elapsed_since_main_start()
{
    return program_timer.get_total_time() / 1e9;
}
...
public:
    ...
    void partition()
    {
        ...
        vertex_count = read_int<ll>(fin, end);
        edge_count = read_int<ll>(fin, end);
        ...
        for (int i = 1; i <= vertex_count; i++)
        {
            ...
            int cur_vertex_id = read_int<int>(fin, end);
            int neighbour_count = read_int<int>(fin, end);
        ...
        refine_timer.log();
        result.core_time =
            stream_timer.get_total_time() + refine_timer.get_total_time();
...
//                        << "\n";
    ...
    void write_metrics_json(long long edge_cut, double node_balance, double edge_balance)
    {
        const string run_dir =
            "partitioned_files/" + dataset_name + "_" + to_string(part_count) + "_" +
            to_string(sub_part_count) + ".artifacts";
        std::filesystem::create_directories(run_dir);
        ofstream out(run_dir + "/metrics.json");
        out << "{\n";
        out << "  \"algorithm\": \"cuttana_single_thread\",\n";
        out << "  \"dataset\": \"" << dataset_name << "\",\n";
        out << "  \"k\": " << part_count << ",\n";
        out << "  \"partitioning_type\": \"node\",\n";
        out << "  \"num_parts\": " << part_count << ",\n";
        out << "  \"num_nodes\": " << vertex_count << ",\n";
        out << "  \"num_edges\": " << edge_count << ",\n";
        out << "  \"edge_cut\": " << edge_cut << ",\n";
        out << "  \"edge_cut_ratio\": "
            << (edge_count ? static_cast<double>(edge_cut) / edge_count : 0.0) << ",\n";
        out << "  \"node_balance\": " << node_balance << ",\n";
        out << "  \"edge_balance\": " << edge_balance << ",\n";
        out << "  \"core_time\": " << elapsed_since_main_start() << "\n";
        out << "}\n";
    }
    ...
    void verify()
    {
        ...
        write_metrics_json(edge_cut, imbalance_vertex, imbalance_edge);
