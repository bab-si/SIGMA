...
#include "extern/VieClus/lib/parallel_mh_clustering/exchange/exchanger_clustering.h"
...
void exchanger_clustering::diversify_population_clustering( KaHIP::PartitionConfig & config, KaHIP::graph_access & G,  population_clustering & island, bool replace ) {
        ...
        if( rank == ROOT ) {
                KaHIP::random_functions::circular_permutation(permutation); 
        }
...
void exchanger_clustering::push_best( KaHIP::PartitionConfig & config, KaHIP::graph_access & G, population_clustering & island ) {
        ...
        if(something_todo) {
                ...
                int target = rank;
                while( target == rank && m_allready_send_to[target]) target = KaHIP::random_functions::nextInt(0, size-1);
