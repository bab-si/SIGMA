...
inline void configuration::standard(PartitionConfig &partition_config) {
    ...
    partition_config.epsilon_edge                           = 10;
    ...
    partition_config.fennel_gamma = 2.5;
    ...
    partition_config.max_cluster_node_volume = std::numeric_limits<unsigned int>::max();
    ...
    // for now 2 constraints number of edges and number of nodes in each partition
    partition_config.number_of_constraints = 2;
    partition_config.stream_blocks_load = NULL;

    partition_config.node_degrees = NULL;
    partition_config.max_load_nodes_part_hdrf = 0;
    partition_config.max_load_edges_part_hdrf = 0;

    partition_config.prepartition_graph = -1;

    partition_config.num_capacity_changes = 5;
    partition_config.node_capacity_change_interval = 0;
    partition_config.edge_capacity_change_interval = 0;

    partition_config.edges_streamed = 0;

    partition_config.partitioning_type = NODE_PARTITIONING;
    partition_config.stream_edges_assign = nullptr;

    partition_config.tau_mult_obj = 0.5;
    
    partition_config.alpha_hdrf = 0.5;
    partition_config.lambda_hdrf = 1;

    partition_config.mult_capacity_y = 1.0;
    partition_config.stream_progress = 0.0;
    partition_config.use_capacity_curve = true;
    partition_config.capacity_min_cap = 0.90;
    partition_config.capacity_alpha = 0.5;

    partition_config.with_aw = false;
    partition_config.stream_nodes_aw = NULL;
}
