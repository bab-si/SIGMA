...
class buffered_input {
	public:
		...
		inline void simple_scan_line(std::vector<LongNodeID>& vec) {
			vec.clear();
			vec.reserve((*this->lines)[this->row].size() / 2 + 1);
			this->column = 0;
			LongNodeID item = 0;
	...
	private:
		...
		inline bool next_int(T& item) {
			...
			item = 0;
            const std::string& current_line = (*this->lines)[this->row];
            const size_t line_size = current_line.size();
            const char* const data = current_line.data();

			while (this->column < line_size) {
				const char c = data[this->column];
				switch(c) {
