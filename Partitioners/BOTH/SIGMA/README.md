...
- Enforces **both** vertex balance and edge-volume balance simultaneously as hard capacity constraints.
- Supports **both** vertex and edge partitioning modes.
- Optionally optimizes **both** edge cut and replication factor simultaneously in a single scoring function.
...
### Phase 1 (Optional): Streaming Clustering (CluStRE)
...
Source: `lib/partition/pre_partitioner/clustre_prepartition.{h,cpp}`
...
Source: `lib/partition/pre_partitioner/prepartition.{h,cpp}`
...
**Vertex partitioning - Normalized Fennel:**
```
S_vertex(v, p) = e(v,p) / deg(v)  −  ρ_p^(γ−1.1)
```
where `e(v,p)` is the edge weight to already-assigned neighbors in `p`, and `ρ_p = max(vertex_load/vertex_capacity, vol_load/vol_capacity)` is the most-loaded constraint dimension.
...
**Edge partitioning - Extended HDRF:**
```
...
Source: `lib/partition/onepass_partitioning/`
...
| `--one_pass_algorithm range_fennel\|fennel_mult_obj\|hdrf` | Scoring function |
...
| `--imbalance <float>` | Vertex balance tolerance ε |
...
```bash
./deploy/sigma /path/to/cit-patent.graph -k 128 \
...
```bash
./deploy/sigma /path/to/ljournal-2008.graph -k 8 \
...
Process FlatBuffer result files into CSV:
```bash
python3 create_csv_files.py
python3 eval_scripts/evaluate_fb_files.py
```
