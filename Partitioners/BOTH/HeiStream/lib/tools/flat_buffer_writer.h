...
class FlatBufferWriter {
   private:
    ...
    double node_edge_balance_;
    double edge_node_balance_;
   ...
   public:
    FlatBufferWriter()
        : io_time_(0.0),
          ...
          balance_(0.0),
          node_balance_(0.0),
          node_edge_balance_(0.0),
          edge_node_balance_(0.0) {}
    ...
    void updateVertexPartitionResults(EdgeWeight& edge_cut, double node_balance,
                                      double edge_balance) {
        edge_cut_ = edge_cut;
        balance_ = node_balance;
        node_balance_ = node_balance;
        node_edge_balance_ = edge_balance;
    }

    void updateEdgePartitionResults(NodeID& vertex_cut, NodeID& replicas, double replication_factor,
                                    double balance, double node_balance) {
        vertex_cut_ = vertex_cut;
        replicas_ = replicas;
        replication_factor_ = replication_factor;
        balance_ = balance;
        edge_node_balance_ = node_balance;
    }
    ...
    void write_json(const std::string& graph_filename, const Config& partition_config) const {
        std::string dataset = ::extractBaseFilename(graph_filename);
        std::string output_dir = partition_config.filename_output.empty()
                                     ? dataset + "_" + std::to_string(partition_config.k) + ".artifacts"
                                     : partition_config.filename_output + ".artifacts";
        std::filesystem::create_directories(output_dir);
        std::string output_path = output_dir + "/metrics.json";

        std::ofstream out(output_path, std::ios::out | std::ios::trunc);
        out << "{\n";
        out << "  \"algorithm\": \"heistream\",\n";
        out << "  \"dataset\": \"" << dataset << "\",\n";
        out << "  \"k\": " << partition_config.k << ",\n";
        out << "  \"partitioning_type\": \""
            << (partition_config.edge_partition ? "edge" : "node") << "\",\n";
        out << "  \"num_parts\": " << partition_config.k << ",\n";
        out << "  \"num_nodes\": " << partition_config.total_nodes << ",\n";
        out << "  \"num_edges\": " << partition_config.total_edges << ",\n";
        if (partition_config.edge_partition) {
            out << "  \"node_balance\": " << edge_node_balance_ << ",\n";
            out << "  \"edge_balance\": " << balance_ << ",\n";
            out << "  \"replication_factor\": " << replication_factor_ << ",\n";
        } else {
            out << "  \"edge_cut\": " << edge_cut_ << ",\n";
            out << "  \"edge_cut_ratio\": "
                << (partition_config.total_edges > 0
                        ? edge_cut_ / static_cast<double>(partition_config.total_edges)
                        : 0.0)
                << ",\n";
            out << "  \"node_balance\": " << node_balance_ << ",\n";
            out << "  \"edge_balance\": " << node_edge_balance_ << ",\n";
        }
        out << "  \"core_time\": " << total_time_ << "\n";
        out << "}\n";
    }
