...
void NodePartitioningEvaluator::evaluate(Config& config, const std::string& graph_filename,
    ...
    partition::metrics::node::evaluate_partition(config, *ctx.node_pass_state.stream_nodes_assign,
                                                 graph_filename, edge_cut, edge_balance);
    quality_metrics qm;
    const double node_balance = qm.balance_full_stream(*ctx.node_pass_state.stream_blocks_weight);
    fb_writer.updateVertexPartitionResults(edge_cut, node_balance, edge_balance);
}
