...
void evaluate_partition(const Config& config, const std::vector<PartitionID>& stream_nodes_assign,
                        const std::string& filename, EdgeWeight& edgeCut, double& edgeBalance) {
    ...
    std::vector<EdgeWeight> partition_edge_weight(config.k, 0);
    edgeCut = 0;
    edgeBalance = 0.0;
    ...
    while (std::getline(in, lines[0])) {
        ...
        while (col_counter < line_numbers.size()) {
            ...
            partition_edge_weight[partitionIDSource] += edge_weight;
    ...
    if (total_edgeweight > 0 && config.k > 0) {
        const auto max_weight = *std::max_element(partition_edge_weight.begin(),
                                                  partition_edge_weight.end());
        edgeBalance = max_weight / (total_edgeweight / static_cast<double>(config.k));
    }
}
